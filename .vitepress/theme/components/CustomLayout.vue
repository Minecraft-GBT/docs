...
function getText(key) {
  return translates[lang.value][key]
}
</script>
    ...
    <template #doc-before v-if="frontmatter.download">
      <div class="vp-doc external-link-icon-enabled"> 
        ...
        <p class="icons-flex download-button" style="height: auto; flex-wrap: wrap;">
          <a v-if="frontmatter.link" primary aria-label="Download" :href="frontmatter.link" target="_blank" rel="noreferrer"><svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1m-4-4-4 4m0 0-4-4m4 4V4"></path></svg> {{ getText('download_selfhost') }} </a>
        ...
        <p>{{ getText('modification_1') }} <a href="mailto:gbt@gteh.top">gbt@gteh.top</a> {{ getText('modification_2') }}</p>
        <h2 :id="getText('version_info_code')" tabindex="-1">{{ getText('version_info') }} <a class="header-anchor" :href="'#' + getText('version_info_code')" :aria-label="'Permalink to &quot;' + getText('version_info') + '&quot;'">​</a></h2>
        <p v-if="frontmatter.channel" class="icons-flex"><b>{{ getText('release_channel') }}</b>:&nbsp;<span :class="'circle circle-' + frontmatter.channel"></span> <span :class="frontmatter.channel"><b>{{ frontmatter.channel }}</b></span></p>
        <p v-if="frontmatter.date"><b>{{ getText('publication_date') }}</b>:&nbsp;{{ frontmatter.date }}</p>
        <p v-if="frontmatter.loaders" class="icons-flex loader-icon"><b>{{ getText('loaders') }}</b>:&nbsp;<p v-for="loader in frontmatter.loaders">
        ...
        <h2 :id="getText('changelog_code')" tabindex="-1">{{ getText('changelog') }} <a class="header-anchor" :href="'#' + getText('changelog_code')" :aria-label="'Permalink to &quot;' + getText('changelog') + '&quot;'">​</a></h2>
      </div>
    </template>
    <!-- <template #doc-footer-before v-if="frontmatter.download">
      <div class="vp-doc external-link-icon-enabled">
        <h2 :id="getText('files_code')" tabindex="-1">{{ getText('files') }} <a class="header-anchor" :href="'#' + getText('files_code')" :aria-label="'Permalink to &quot;' + getText('files') + '&quot;'">​</a></h2>
        <p>{{ getText('click_to_download') }}</p>
        <div v-for="file in frontmatter.files">
          <p><a :href="frontmatter.base + file.link" target="_blank" rel="noreferrer">{{ file.name }}</a> ({{ file.size }})<span v-if="file.primary"> ({{ getText('primary_file') }})</span></p>
        </div>
      </div>
    </template> -->
    <template #doc-after>
