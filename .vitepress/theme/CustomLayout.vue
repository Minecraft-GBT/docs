...
const dl_text = {
  'en': {
    ...
    'modification_2': 'for modification.',
    'version_info': 'Version Information',
    'version_info_code': 'version-information',
  ...
  'zh': {
    ...
    'modification_2': '修改。',
    'version_info': '版本信息',
    'version_info_code': '版本信息',
    'release_channel': '更新类型',
...
function getDlTrans(key) {
  return dl_text[lang.value][key]
}
</script>
  ...
  <Layout>
    <template #doc-before v-if="frontmatter.download">
      <div class="vp-doc external-link-icon-enabled"> 
        ...
        <p>{{ getDlTrans('modification_1') }} <a href="mailto:gbt@gteh.top">gbt@gteh.top</a> {{ getDlTrans('modification_2') }}</p>
        <h2 :id="getDlTrans('version_info_code')" tabindex="-1">{{ getDlTrans('version_info') }} <a class="header-anchor" :href="'#' + getDlTrans('version_info_code')" :aria-label="'Permalink to &quot;' + getDlTrans('version_info') + '&quot;'">​</a></h2>
        <p v-if="frontmatter.channel" class="icons-flex"><b>{{ getDlTrans('release_channel') }}</b>:&nbsp;<span :class="'circle circle-' + frontmatter.channel"></span> <span :class="frontmatter.channel"><b>{{ frontmatter.channel }}</b></span></p>
        <p v-if="frontmatter.date"><b>{{ getDlTrans('publication_date') }}</b>:&nbsp;{{ frontmatter.date }}</p>
        <p v-if="frontmatter.loaders" class="icons-flex loader-icon"><b>{{ getDlTrans('loaders') }}</b>:&nbsp;<p v-for="loader in frontmatter.loaders">
        ...
        <p v-if="frontmatter.dep"><b>{{ getDlTrans('dependencies') }}</b>:&nbsp;<span v-for="dep in frontmatter.dep">
          <span style="margin-right: 12px;">{{ dep }}</span>
        </span></p>
        <h2 :id="getDlTrans('changelog_code')" tabindex="-1">{{ getDlTrans('changelog') }} <a class="header-anchor" :href="'#' + getDlTrans('changelog_code')" :aria-label="'Permalink to &quot;' + getDlTrans('changelog') + '&quot;'">​</a></h2>
      </div>
    </template>
    <template #doc-footer-before v-if="frontmatter.download">
      <div class="vp-doc external-link-icon-enabled">
        <h2 :id="getDlTrans('files_code')" tabindex="-1">{{ getDlTrans('files') }} <a class="header-anchor" :href="'#' + getDlTrans('files_code')" :aria-label="'Permalink to &quot;' + getDlTrans('files') + '&quot;'">​</a></h2>
        <p>{{ getDlTrans('click_to_download') }}</p>
        <div v-for="file in frontmatter.files">
          <p><a :href="frontmatter.base + file.link" target="_blank" rel="noreferrer">{{ file.name }}</a> ({{ file.size }})<span v-if="file.primary"> ({{ getDlTrans('primary_file') }})</span></p>
        </div>
      </div>
    </template>
    <template #doc-after>
